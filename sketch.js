...
function preload(){
  img = loadImage('output_v9S6fC.gif');
  curwenfont = loadFont('CURWENFONT.ttf');
  source = loadSound('KWORIENALL.ogg', loaded, error, whileLoading);
}
...
function setup() {
  ...
  document.addEventListener("mousedown", ()=>{if (source.isPlaying()) {
    source.pause();
  } else {
    source.loop();
  }}) 
